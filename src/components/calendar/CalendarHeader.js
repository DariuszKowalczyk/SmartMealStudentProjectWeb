  ...
  <>
    <Row className="d-flex justify-content-center">
      <Col sm={1} className="d-flex align-items-center justify-content-end">
        <IoIosArrowBack onClick={setPreviousWeek} size={25} class="calendar-header-arrow-icon" />
      </Col>
      <Col sm={7}>
        <Row className="no-gutters flex-row">
          {days &&
            days.map((day, idx) => (
              <Col className="calendar-header-col" onClick={() => setActive(idx)}>
                <span>
      ...
      <Col sm={1} className="d-flex align-items-center justify-content-start">
        <IoIosArrowForward onClick={setNextWeek} size={25} class="calendar-header-arrow-icon" />
      </Col>
    </Row>
    <Row>
